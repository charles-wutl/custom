/**
 * publish_jsPullUpToRefresh.js
 * Created on 2016/5/19 18:11
 * Tim
 * dajia-application
 ...
 * */
function publish_jsPullUpToRefresh(option){
    this.setting = {
        loadingDiv:$('<p class="mCommon_jsPullToRefresh_loading "><img class="mCommon_jsPullToRefresh_loading_icon"  src="../../../../../modulev1/portalHtml5/images/mCommon_basicIcon_loading.png"/>加载中</p>'),//加载中显示的jq对象
        noneMoreDiv:$("<p class='mCommon_jsPullToRefresh_noneMore'>没有更多内容</p>"),//没有更多内容显示的jq对象
        listOuterBox:null,//jquery对象,列表外容器容器，找底边位置用
        loadLineUpY:20,//加载底边的向上偏移量 int型
    ...
    this.loadingBox_showHide=function(){
        this.boxH=this.setting.listOuterBox.outerHeight();//列表盒子高度
        if(this.boxOffsetTop+this.boxH < this.winH){//列表盒子的底线在 一屏之内
            this.loadingBox.hide();//loadingBox隐藏
        }else{//列表盒子的底线在 一屏之外
            this.loadingBox.show(100);//loadingBox显示
        }
    ...
    this.appendData=function(){
        ...
        self.loadingBox_showHide();//loadingBox是否显示
